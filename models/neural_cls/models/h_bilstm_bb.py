...
from models.neural_cls.modules import EncoderRNN_BB
from models.neural_cls.util.utils import *


class h_BiLSTM_BB(nn.Module):
    ...
    def __init__(self, word_vocab_size, word_embedding_dim, word_hidden_dim, output_size, sigma_prior, 
        ...
        super(h_BiLSTM_BB, self).__init__()
    ...
    def forward(self, words, tags, wordslen, n_batches, n_samples = 3, usecuda=True):
        
        batch_size, max_len = words.size()
        s_log_pw, s_log_qw, s_log_likelihood = 0., 0., 0.
                
        if usecuda:
            onehottags = Variable(torch.zeros(batch_size, self.output_size)).cuda()
        else:
            onehottags = Variable(torch.zeros(batch_size, self.output_size))
